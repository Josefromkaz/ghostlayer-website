...
ipcMain.handle('license:canUseFeature', (_, feature: string) => {
  return licenseManager.canUseFeature(feature as 'memory' | 'export' | 'prompts');
});
...
ipcMain.handle('db:addRule', (_, pattern: string, category?: string) => {
...
ipcMain.handle('db:addWhitelistItem', (_, phrase: string) => {
...
ipcMain.handle('db:addCustomPattern', (_, pattern: { name: string; regex: string; category: string }) => {
