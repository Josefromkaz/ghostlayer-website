...
import { X, Plus, Trash2, Brain, Regex, ShieldCheck, Globe, Type } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
  const [activeTab, setActiveTab] = useState<'MEMORY' | 'CUSTOM' | 'WHITELIST' | 'LANGUAGE' | 'APPEARANCE'>('MEMORY');
  ...
  const handleAddCustom = () => {
    if (newCustomName && newCustomRegex) {
      store.addCustomPattern(newCustomName, newCustomRegex);
      setNewCustomName('');
      setNewCustomRegex('');
    }
  };
  ...
  const handleAddWhitelist = () => {
    if (newWhitelistItem) {
      store.addToWhitelist(newWhitelistItem);
      setNewWhitelistItem('');
    }
  };
                        ...
                        {fonts.map(font => (
                            ...
                            >
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xs opacity-60 mt-1">{'<Component prop={value} />'}</span>
                                </div>
              ...
              <div className="flex gap-2">
               ...
               <div className="grid gap-2">
                  {store.whitelist.length === 0 ? (
                    <div className="text-center text-slate-400 py-10 italic">{t.noWhitelist}</div>
                  ) : (
                    store.whitelist.map(item => (
                      <div key={item} className="flex items-center justify-between bg-white dark:bg-slate-800 p-3 rounded shadow-sm border border-slate-200 dark:border-slate-700">
