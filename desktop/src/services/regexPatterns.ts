...
import { luhnCheck, validateKzIin, validateRuInn, validateUsSsn } from './validators';
...
export const CATEGORY_COLORS: Record<RedactionCategory, string> = {
  ...
  [RedactionCategory.SSN]: 'bg-indigo-100 text-indigo-800 border-indigo-300',
  [RedactionCategory.EIN]: 'bg-violet-100 text-violet-800 border-violet-300',
  [RedactionCategory.ROUTING_NUMBER]: 'bg-cyan-100 text-cyan-800 border-cyan-300',
  [RedactionCategory.DRIVER_LICENSE]: 'bg-red-100 text-red-800 border-red-300',
};
...
export const SYSTEM_PATTERNS: PatternConfig[] = [
  ...
  // English Context Persons
  {
    ...
    // Updated to allow dots (e.g. "Sarah E. Thompson") and single letter initials
    // Capture Group 1: The Full Name
    // Use [ \t] instead of \s to avoid capturing newlines (e.g. "John Doe\nAnd")
    regex: /(?:Represented\s+by|Project\s+Lead|Manager|CEO|Officer|Partner|Secretary|Contact|Name|Witnessed\s+By|Cardholder|By)\s*:?\s+((?:[A-Z][a-z.]+[ \t]+){1,3}[A-Z][a-z.]+)/g,
    color: CATEGORY_COLORS[RedactionCategory.PERSON_NAME],
    replacement: '[PERSON_NAME]',
    useGroup1AsIdentity: true,
  },
  // Name followed by Title (e.g. "John Doe, CEO")
  {
    id: 'person_suffix_title',
    name: 'Name (Suffix Title)',
    category: RedactionCategory.PERSON_NAME,
    // Look for 2-3 capitalized words (allowing dots) followed by a comma OR NEWLINE and a known title
    // Allow up to 3 extra words before the main title keyword (e.g. "Data Analytics Manager")
    // Capture Group 1: The Name
    // Use [ \t] for name parts
    regex: /\b((?:[A-Z][a-z.]+[ \t]+){1,2}[A-Z][a-z.]+)(?=\s*[,]\s*(?:[A-Za-z]+\s+){0,3}(?:CEO|CFO|CTO|VP|President|Manager|Director|Partner|Secretary|Chief|Head|Lead|Scientist|Engineer|Officer|Founder|Co-Founder|Owner|Representative|Consultant|Specialist|Analyst|Associate|Administrator|Assistant|Executive|Managing|Senior|Junior)|\s*[\r\n]+\s*(?:[A-Za-z]+\s+){0,3}(?:CEO|CFO|CTO|VP|President|Manager|Director|Partner|Secretary|Chief|Head|Lead|Scientist|Engineer|Officer|Founder|Co-Founder|Owner|Representative|Consultant|Specialist|Analyst|Associate|Administrator|Assistant|Executive|Managing|Senior|Junior))/g,
    color: CATEGORY_COLORS[RedactionCategory.PERSON_NAME],
    replacement: '[PERSON_NAME]',
    useGroup1AsIdentity: true,
  },
  ...
  // Также ловит падежные формы: Нурланова Ерлана Кайратовича, Касымовой Айгуль Маратовной
  {
    id: 'fio_patronymic',
  ...
  // English Companies
  {
    ...
    name: 'Company (EN)',
    category: RedactionCategory.COMPANY,
    // Fix: Remove \b after suffixes that might end with a dot, or handle it correctly
    regex: /\b(?:[A-Z][a-zA-Z0-9&]+\s+)+(?:Inc\.|Ltd\.|Co\.|Corp\.|LLC|Corporation|Limited|Company|Bank)/g,
    color: CATEGORY_COLORS[RedactionCategory.COMPANY],
    replacement: '[COMPANY]',
  },
  {
  ...
  },
  {
    ...
    name: 'URL (www)',
    category: RedactionCategory.URL,
    // Catch www.example.com without http prefix
    regex: /\bwww\.[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&//=]*)/g,
    color: CATEGORY_COLORS[RedactionCategory.URL],
    replacement: '[URL]',
  },
  {
    ...
    name: 'URL (Domain)',
    category: RedactionCategory.URL,
    // Catch domain.com/path where domain is common (linkedin, github, etc) or standard structure
    // We'll be a bit specific to avoid false positives with regular text
    regex: /\b(?:linkedin|github|facebook|twitter|instagram|tiktok|youtube|vimeo|medium|substack)\.com\/[-a-zA-Z0-9()@:%_+.~#?&//=]*/gi,
    color: CATEGORY_COLORS[RedactionCategory.URL],
    replacement: '[URL]',
  },
  {
    id: 'employee_id',
    name: 'Employee/Staff ID',
    category: RedactionCategory.ID_DOCUMENT,
    regex: /(?:Employee|Staff|Worker)\s*(?:ID|Code|No\.?|Number|#)\s*:?\s*([A-Z0-9-]{3,15})/gi,
    color: CATEGORY_COLORS[RedactionCategory.ID_DOCUMENT],
    replacement: '[EMPLOYEE_ID]',
    useGroup1AsIdentity: true,
  },
  {
    id: 'address_us_street',
    name: 'Address (US Street)',
    category: RedactionCategory.ADDRESS,
    // Start of line or boundary, number, then words, then Street Type
    regex: /\b\d{1,5}\s+(?:[A-Z][a-z0-9]+\s+){1,4}(?:Street|St|Avenue|Ave|Drive|Dr|Road|Rd|Blvd|Lane|Ln|Way|Plaza|Sq|Court|Ct|Circle|Cir)\b(?:.{1,20}?)?/gi,
    color: CATEGORY_COLORS[RedactionCategory.ADDRESS],
    replacement: '[ADDRESS_LINE]',
  },
  {
    id: 'credit_card',
    name: 'Credit Card',
...
/**
 * Returns available regex patterns based on current license.
 * FREE version gets only 5 basic categories.
 * PRO/TRIAL/TEAM gets all 39 categories.
 */
export async function getAvailablePatterns(): Promise<PatternConfig[]> {
  if (typeof window === 'undefined' || !window.ghostlayer) {
    // Node.js test environment or browser dev mode - all patterns available
    return SYSTEM_PATTERNS;
  }

  const filteredPatterns: PatternConfig[] = [];
  for (const pattern of SYSTEM_PATTERNS) {
    const isAllowed = await window.ghostlayer.license.isRegexCategoryAllowed(pattern.category);
    if (isAllowed) {
