...
export const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
  ...
  const handleAddCustom = async () => {
    if (newCustomName && newCustomRegex) {
      if (!isRegexSafe(newCustomRegex)) {
        alert('Invalid or unsafe regular expression. Please simplify it (avoid nested quantifiers like (a+)+).');
        return;
      }
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Basic Redaction (5 categories)</span>
                    <span className="text-green-600">✓ Included</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Advanced Detection (35+ categories)</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Memory Rules</span>
                    <span className={licenseInfo?.type === 'PRO' || licenseInfo?.type === 'TEAM' || licenseInfo?.type === 'TRIAL' ? 'text-green-600' : 'text-slate-400'}>
                      {licenseInfo?.type === 'PRO' || licenseInfo?.type === 'TEAM' || licenseInfo?.type === 'TRIAL' ? '✓ Included' : 'PRO Only'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Whitelist</span>
                    <span className={licenseInfo?.type === 'PRO' || licenseInfo?.type === 'TEAM' || licenseInfo?.type === 'TRIAL' ? 'text-green-600' : 'text-slate-400'}>
                      {licenseInfo?.type === 'PRO' || licenseInfo?.type === 'TEAM' || licenseInfo?.type === 'TRIAL' ? '✓ Included' : 'PRO Only'}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Export Results</span>
                    <span className="text-green-600">✓ Included</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Saved Prompts</span>
                    <span className="text-green-600">✓ Included</span>
                  </div>
                </div>
              </div>
